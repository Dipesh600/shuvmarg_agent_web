...
export default function FilterSidebar({ results, filters, onFiltersChange }: FilterSidebarProps) {
    ...
    >
      <div
        className={`w-5 h-5 rounded-[4px] border flex items-center justify-center transition-colors flex-shrink-0 ${
          checked
            ? "bg-[#7A1D1B] border-[#7A1D1B]"
            : "border-neutral-300 bg-white group-hover:border-[#7A1D1B]"
        }`}
      >
  ...
  return (
    <div className="bg-white rounded-[24px] shadow-sm border border-neutral-100 overflow-hidden h-max sticky top-8">
      {/* Header */}
      <div className="px-5 py-4 border-b border-neutral-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h3 className="text-[16px] font-black text-neutral-900">Filters</h3>
          {activeCount > 0 && (
            <span className="text-[11px] font-bold bg-[#7A1D1B] text-white px-2 py-0.5 rounded-full">
              {activeCount}
            </span>
          )}
        </div>
        {activeCount > 0 && (
          <button
            className="text-[13px] font-bold text-[#7A1D1B] hover:text-[#5C1414] transition-colors"
            onClick={clearAll}
          >
            Clear All
          </button>
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1">
                  ...
                  <input
                    ...
                    className="w-full h-10 px-3 border border-neutral-300 rounded-lg text-[14px] font-semibold focus:outline-none focus:border-[#7A1D1B]"
                  />
                </div>
