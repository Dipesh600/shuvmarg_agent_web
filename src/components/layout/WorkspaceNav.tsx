...
export default function WorkspaceNav() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      const windowScrolled = window.scrollY > 20;
      const elScrolled = el ? el.scrollTop > 20 : false;
      setScrolled(windowScrolled || elScrolled);
    };

    // Attach to window for mobile scrolling
    window.addEventListener("scroll", handleScroll, { passive: true });

    const timer = setTimeout(() => {
      // Attach to desktop scroll container
      el = document.querySelector(".workspace-scroll-area") as HTMLElement | null;
      if (el) {
  ...
  return (
    <>
      ...
      */}
      <header
        className={`hidden md:flex fixed top-0 left-0 right-0 h-20 z-50 items-center px-4 lg:px-6 justify-between pointer-events-none transition-all duration-500 ease-out ${
          scrolled
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-shrink-0 w-60 pl-2 pointer-events-auto">
          <Link href="/dashboard" className="flex items-center gap-2">
            ...
            <span className="font-black text-[28px] tracking-tighter flex items-baseline">
      ...
      */}
      ...
      >
        ...
        <div className="mt-auto pt-4 flex flex-col items-center lg:items-stretch gap-4 lg:gap-2 w-full lg:px-2 border-t border-white/10">

          <div className="relative w-full flex justify-center lg:justify-start">
            <div 
              className="w-12 h-12 lg:w-full lg:h-auto flex items-center gap-3 lg:p-2 rounded-full lg:rounded-xl hover:bg-white/10 cursor-pointer transition-colors"
              onClick={() => setAvatarMenuOpen(!avatarMenuOpen)}
            >
              <div className="w-12 h-12 lg:w-10 lg:h-10 rounded-full bg-white text-[#D96B62] flex items-center justify-center flex-shrink-0 shadow-sm transition-transform hover:scale-105 lg:hover:scale-100 overflow-hidden relative">
                {agentProfile?.avatar ? (
                  <img src={agentProfile.avatar} alt={agentProfile.name || "Avatar"} className="w-full h-full object-cover" />
                ) : (
                  <span className="text-[14px] font-bold">{agentProfile?.initials || "ST"}</span>
                )}
              </div>
              <div className="hidden lg:block flex-1 overflow-hidden">
                <p className="text-[14px] font-bold text-white truncate">{agentProfile?.name || "Partner"}</p>
                <p className="text-[12px] text-white/70 truncate">ID: {agentProfile?.id || "N/A"}</p>
              </div>
            </div>
            ...
            <AnimatePresence>
              {avatarMenuOpen && (
                <motion.div
                  ...
                  className="absolute bottom-0 left-16 lg:bottom-full lg:left-0 lg:mb-2 ml-2 lg:ml-0 w-56 bg-white border border-neutral-200 rounded-xl shadow-xl overflow-hidden z-50"
                >
                  <div className="p-1">
                    <div className="px-3 py-3 border-b border-neutral-100 mb-1 lg:hidden">
      ...
      */}
      <header
        className={`md:hidden fixed top-0 left-0 right-0 h-16 z-50 flex items-center justify-between px-4 transition-all duration-500 ease-out ${
          scrolled
            ? "bg-[#FAF7F2]/90 backdrop-blur-2xl border-b border-neutral-200/60 shadow-[0_1px_20px_rgba(0,0,0,0.06)]"
            : "bg-transparent"
        }`}
      >
        <Link href="/dashboard" className="flex items-center gap-2">
          <span className="font-black text-[28px] tracking-tighter flex items-baseline">
            ...
            >Shuv</span>
            <span
              className={`transition-colors duration-500 ${scrolled ? "text-[#D96B62]" : "text-white/90"}`}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <div
              className={`w-8 h-8 rounded-full cursor-pointer flex items-center justify-center shadow-sm transition-all duration-500 overflow-hidden relative ${
                scrolled ? "bg-[#7A1D1B] text-white" : "bg-white text-[#7A1D1B]"
              }`}
              onClick={() => setAvatarMenuOpen(!avatarMenuOpen)}
            >
              {agentProfile?.avatar ? (
                ...
                <span className="text-[10px] font-bold">{agentProfile?.initials || "ST"}</span>
              )}
            </div>
